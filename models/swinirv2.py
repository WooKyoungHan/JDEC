...
from models.plainvit import apply_subblock, patch2subblock, patch2rearrange
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        logit_scale = torch.clamp(self.logit_scale, max=torch.log(torch.tensor(1. / 0.01, device=self.logit_scale.device))).exp() # this needs to be defined on GPU (the torch.tensor part)
...
class PatchEmbedding_DCT_Group(nn.Module):
    """
    Patch embedding layer for DCT. Added `norm_layer' to support SwinV2.
    
    Grouped embedding for YCbCr. Combine Y/CbCr blocks appropriately if `use_subblock' is True.

    img_size: Input image size (in terms of RGB)
    ...
    norm_layer: If set, apply this normalization layer after projection
    ...
    """
    def __init__(
            self, img_size=256, patch_size: int = 16, # Input: (B, nC, H, W, 8, 8)
            emb_size: int = 768, use_subblock=True,
            norm_layer=None, chroma_scale=2,
        ...
        self.patches_resolution = [img_size // patch_size, img_size // patch_size] # swinv2 specific
        self.num_patches = self.patches_resolution[0] * self.patches_resolution[1] # swinv2 specific
    ...
    def forward(self, y, cbcr) -> Tensor:
        b, _, _, _, _, _ = y.shape
        if self.conv_Y is not None:
            self.conv_Y =self.conv_Y.cuda()#
        if self.conv_C is not None:
            self.conv_C =self.conv_C.cuda()#
        if self.combine_Y:
...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        x = x.transpose(1, 2).reshape(B, -1 ,x_size[0], x_size[1])  # B Ph*Pw C
        # print(x.shape)
        return x
...
class SwinTransformerV2_Ycbcr_ir_2(nn.Module):
    r""" Swin Transformer
    ...
    Args:
        ...
        device: device to initialize SwinV2 model to. ('cpu', 'cuda', gpu_rank, ...)
        pixel_space: 'rgb' or 'dct'. Defines SwinV2 model in the given pixel space
    """
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 device='cpu', pixel_space='dct', **kwargs):
        super().__init__()
        ...
        self.mlp_ratio = mlp_ratio
        self.pixel_space = pixel_space.lower() # 'rgb' or 'dct'

        # split image into non-overlapping patches
        if pixel_space.lower() == "rgb":
            self.patch_embed = PatchEmbed(
                img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
                norm_layer=norm_layer if self.patch_norm else None)
        elif pixel_space.lower() == 'dct':
            self.patch_embed = PatchEmbedding_DCT_Group(
            img_size=img_size, patch_size=4, emb_size=embed_dim, use_subblock=True, norm_layer=norm_layer, device=device,
            )
        ...
        self.patch_unembed = PatchUnEmbed(
            img_size=img_size, patch_size=4, in_chans=embed_dim, embed_dim=4*embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)
        ...
        self.patch_embed_deep = PatchEmbed(
        ...
        self.layers_RSTB = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer_RSTB = RSTB(dim=embed_dim,
            ...
            self.layers_RSTB.append(layer_RSTB)


        self.norm = norm_layer(embed_dim)
        self.out_dim = embed_dim
        self.conv_after_body = nn.Conv2d(embed_dim,self.out_dim,3,1,1)
    ...
    def pre_forward_features(self, y,cbcr):
        self.x_size = (y.shape[2]*2, y.shape[3]*2)
        # print(self.x_size)
        if self.pixel_space == 'rgb':
            x = self.patch_embed(y)
        elif self.pixel_space == 'dct':
            x = self.patch_embed(y,cbcr)
        if self.ape:
    ...
    def forward(self, y,cbcr):
        ...
        x = x.permute(0,2,3,1).reshape(-1,self.x_size[0]*self.x_size[1],self.embed_dim)
        x = x+ xl
        return x
